...
export async function getServerSideProps({ query }) {
  ...
  const { game, games_page, game_aggregated } = (
    await client.query({
      query: gQuery,
      variables: {
  ...
  const carousel = games_page.hero_carousel
    ? games_page.hero_carousel.map((item) => ({
...
export default function GamesPage({ games, page, years }) {
  ...
  const paths = [
    { url: '/', label: 'home' },
    { url: '/games', label: 'games', disabled: true },
    {
      url: `/games/${year}`,
      label: year,
      disabled: true,
    ...
    <>
      ...
      <BannerBreadcrumb paths={paths} images={page.carousel}>
        <p className="text-5xl text-neutral-100 text-center uppercase font-semibold">
          {page.hero_title
            ? stringBind(page.hero_title, 'year', year)
            : `Games ${year}`}{' '}
        </p>
      </BannerBreadcrumb>
      <Container>
      ...
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
          {games.map((game, index) => (
            <div key={index} className="border border-text-neutral-300">
              <iframe
                src={`https://www.youtube.com/embed/${game.video_url}`}
              ...
              <div className="p-4 flex flex-col justify-between">
                <div>
                  ...
                  <h6 className="mb-2 tracking-tight text-neutral-500">
                    {game.authors.join(', ')}
                  </h6>
                  {game.description && (
      ...
      <GameBannerYears
        section={{ title: 'Veja também os jogos dos outros anos:' }}
        games={years}
