...
export async function getServerSideProps(context) {
  ...
  const query = gql`
      query BlogArticle($id: ID!) {
          recent_news: news (limit: 5, page: 1, sort: "-date_created", filter: {status: {_eq: "published"}}) {
              user_created {
                  avatar {
                      id
                  }
  ...
  const { news_by_id, recent_news } = (await client.query({
    query, variables
  })).data;
...
export default function Index({ news, recent_news: recentPosts }) {
  const cover = !!news.cover ? [{ url: apiAsset(news.cover.id), alt: news.cover.title }] : null;
  ...
  return (
    <>
      <HeadSeo title={`${news.title}`} description={news.description}
               openGraph={news.cover} keywords={keywords.join(', ')} />
            ...
            <div
              ...
              <div className='inline-block text-left'>
                <p className='flex font-light items-center gap-x-2 text-sm'><CalendarIcon
                  ...
                  em {format(new Date(news.date_created), 'dd MMM, yyyy')}</p>
              </div>
            </div>
          </div>
