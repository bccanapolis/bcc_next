...
export default function Index({
  ...
  const cover = !!article?.cover
    ? [{ url: apiAsset(article.cover?.id), alt: article.cover?.title }]
    : null;
  ...
  return (
    <>
      <HeadSeo
        title={`${article.title} - ${article.user_created.full_name}`}
      ...
      <Container className="flex flex-col-reverse lg:flex-row w-full gap-16 lg:gap-x-8">
          ...
          <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
            <div className="inline-flex items-center space-x-2">
              ...
              <div className="inline-block text-left">
                <p className="flex font-medium items-center gap-x-2">
                  <UserIcon className="w-4 h-4 inline" />{' '}
                  {article.user_created.full_name}
                </p>
            ...
            <ul className="inline-flex gap-x-1">
              {!!article.tags &&
                article.tags.map(({ article_tags_id: item }) => (
                  <li
                    key={`tag-post-${article.slug}-${item.name}`}
        ...
        <ArticlePanel
          ...
          pageAuthors={[article.user_created]}
...
export async function getServerSideProps(context) {
  ...
  const { article_by_id, recent_article, article_tags } = (
    await client.query({
      query,
      variables,
    })
