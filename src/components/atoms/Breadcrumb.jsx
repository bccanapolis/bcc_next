  ...
  return (
    <>
      <nav
        ...
        className='flex py-3 px-4 text-neutral-700'>
        <ol className='inline-flex items-center space-x-1 md:space-x-3'>
          {
            paths.map((link, index) => (
              <li key={link.label} aria-current={(index === paths.length - 1) && 'page'}
                ...
                {
                  (index != 0) && <ChevronRightIcon className='mr-2 w-4 h-4 text-neutral-100' />
                }
                {
                  !!link.dropdown ?
                    <>
                              ...
                              <li key={index}>
                                {
                                  item.reload ?
                                    <span onClick={() => handleReloadPage(`${window.location.origin}${item.url}`)}
                                          className='block py-2 px-4 text-sm text-neutral-700 hover:bg-neutral-100 cursor-pointer'>{item.label}</span>
                                    :
                                    <Link href={item.url}>
                                      <a
                                        className='block py-2 px-4 text-sm text-neutral-700 hover:bg-neutral-100 cursor-pointer'>{item.label}</a>
                                    </Link>
                                }
                    ...
                    !link.disabled ?
                      ...
                      <>
                        <span
                          className='inline-flex items-center text-sm font-medium text-neutral-300 uppercase'
                        >
                          {link.label}
                        </span>
                      </>
